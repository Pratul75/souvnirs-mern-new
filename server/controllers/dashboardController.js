...
const fetchDashboardCardsData = async (req, res) => {
    ...
    if (role === "vendor") {
        ...
        sales = await Order.find({ order_status: "delivered", vendor_id: req.UserId }).count();
...
const getBarChartData = async (req, res) => {
    ...
    if (role === 'vendor') {
        ...
        const totalSales = await Order.aggregate([
            {
                $match: {
                    order_status: "delivered", vendor_id: userId
                }
            },
            {
        ...
        return res.status(200).json({ dateData: mixeddateData, monthData: mixedMonthData, yearData: mixedYearData, totalSales: totalSales.length > 0 ? totalSales[0].totalSales : 0 })
    ...
    const mixedMonthData = mixArrays(last7Months, m);
    const mixedYearData = mixArrays(last7Years, y);
    res.status(200).json({ dateData: mixeddateData, monthData: mixedMonthData, yearData: mixedYearData, totalSales: totalSales[0].totalSales })
}
